...
void m10_atp_handle_response(struct gsm_pvt* pvt)
{
    if (!pvt->cmd_queue.first) return;

    ast_verbose("rgsm: M10 module not supported now\n");
...
/*
    ...
    if(pvt->cmd_queue.first->oper == AT_OPER_EXEC){
        ...
        switch(pvt->cmd_queue.first->id){
            //++++++++++++++++++++++++++++++++++++++++++++++++++
            case AT_M10_QMIC:
                if(strstr(pvt->recv_buf, "+QMIC:")){
                    pvt->parser_ptrs.m10_qmic_rd = (struct at_m10_qmic_read *)&pvt->parser_buf;
                    if(at_m10_qmic_read_parse(pvt->recv_buf, pvt->recv_len, pvt->parser_ptrs.m10_qmic_rd) < 0){
                        // parsing error
                        ast_log(LOG_ERROR, "<%s>: at_m10_qmic_read_parse(%.*s) error\n", pvt->name, pvt->recv_len, pvt->recv_buf);
                        pvt->recv_buf[0] = '\0';
                        ...
                        pvt->gainout_curr = pvt->parser_ptrs.m10_qmic_rd->headset_mic;
                        // check for query
                        if(pvt->querysig.gainout){
            ...
            case AT_M10_QSIMSTAT:
                if(strstr(pvt->recv_buf, "+QSIMSTAT:")){
                    // parse csmins
                    pvt->parser_ptrs.m10_qsimstat_rd = (struct at_m10_qsimstat_read *)&pvt->parser_buf;
                    if(at_m10_qsimstat_read_parse(pvt->recv_buf, pvt->recv_len, pvt->parser_ptrs.m10_qsimstat_rd) < 0){
                        ast_log(LOG_ERROR, "<%s>: at_m10_qsimstat_read_parse(%.*s) error\n", pvt->name, pvt->recv_len, pvt->recv_buf);
                        pvt->recv_buf[0] = '\0';
                        pvt->recv_buf_valid = 0;
                        pvt->recv_len = 0;
                        }
                    else{
                        if(pvt->parser_ptrs.m10_qsimstat_rd->sim_inserted != pvt->flags.sim_present){
                            ast_verbose("eggsm: <%s>: SIM %s\n", pvt->name, (pvt->parser_ptrs.m10_qsimstat_rd->sim_inserted)?("inserted"):("removed"));
                            if(!pvt->parser_ptrs.m10_qsimstat_rd->sim_inserted){
                        ...
                        pvt->flags.sim_present = pvt->parser_ptrs.m10_qsimstat_rd->sim_inserted;
                        }
                    }
                break;
