#ifndef __RGSM_SIM900_H__
#define __RGSM_SIM900_H__

#include <sys/types.h>
#include "at.h"

#define SIM900_MIC_GAIN_DEFAULT     4
#define SIM900_SPK_GAIN_DEFAULT     100
#define SIM900_SPK_AUDG_DEFAULT     25000
...
enum{
	...
	//STK
	AT_SIM900_PSSTKI = AT_PSSTKI,   //AT*PSSTKI - SIM Toolkit interface configuration
	AT_SIM900_PSSTK = AT_PSSTK,     //AT*PSSTK  - SIM Toolkit control
    ...
    AT_SIM900_AUDG,                 // AT+AUDG undocummented command to set gain for audio channels
    AT_SIM900_SIMEI,                // AT+SIMEI="new_imei" write command to change module IMEI
    AT_SIM900_DDET,                 // AT+DDET=0(disable)/1(enable) - DTMF detection
	//
	AT_SIM900_MAXNUM,
};
//------------------------------------------------------------------------------
...
//at command processor for SIM900
extern void sim900_atp_handle_response(struct gsm_pvt* pvt);

/**
 * rgsm_lock and pvt->lock must not be locked before this call
 * return 0 on success or !=0 on failure
*/
extern int sim900_fw_update(struct gsm_pvt* pvt, char *msgbuf, int msgbuf_len);

/**
 * sim900_init - initialisation of driver
 *
 */

extern int sim900_init(struct gsm_pvt* pvt);

//------------------------------------------------------------------------------
#endif //__RGSM_SIM900_H__
